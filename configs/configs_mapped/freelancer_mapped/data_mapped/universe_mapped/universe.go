...
func Read(input_file *file.File) *Config {
	...
	if bases, ok := iniconfig.SectionMap[KEY_BASE_TAG]; ok {
		for _, base := range bases {
			...
			base_to_add.BGCS_base_run_by = semantic.NewString(base, KEY_BASE_BGCS, semantic.Optional())
	...
	if systems, ok := iniconfig.SectionMap[KEY_SYSTEM_TAG]; ok {
		for _, system := range systems {
			...
			system_to_add.Msg_id_prefix = semantic.NewString(system, KEY_SYSTEM_MSG_ID_PREFIX, semantic.Optional())
