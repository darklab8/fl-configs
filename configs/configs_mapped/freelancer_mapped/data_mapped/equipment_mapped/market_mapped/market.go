...
type MarketGood struct {
	...
	BaseSells     *semantic.Int   // 4 . Base sells if value is above 0
	Wtf5          *semantic.Int   // 5.
	...
	for _, file := range frelconfig.Files {
		...
		for _, section := range file.Sections {
			...
			for good_index, _ := range section.ParamMap[KEY_MARKET_GOOD] {
				...
				good_to_add.BaseSells = semantic.NewInt(section, KEY_MARKET_GOOD, semantic.Index(good_index), semantic.Order(4))
				good_to_add.Wtf5 = semantic.NewInt(section, KEY_MARKET_GOOD, semantic.Index(good_index), semantic.Order(5))
